...
//                  {
//                      //int value = literal->value->getIntValue();
//                      //std::unique_ptr<LiteralExpr> nliteral = std::make_unique<LiteralExpr>(std::make_unique<Literal>(value));
                   
//                  }
  
//              }
//              std::cout<<"Assign: "<< expr->toString() << std::endl;
...
std::unique_ptr<Stmt> Parser::declaration()
{
    ...
    if (match(TokenType::IDBOOL))
    {
     ...
     if (match(TokenType::IDENTIFIER))
     {
         ...
         std::unique_ptr<Expr> value = logic_or();
